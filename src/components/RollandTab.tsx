...
export const RollandTab: React.FC<RollandTabProps> = ({ onToast, allStreamers = [] }) => {
              ...
              {LINE_KEYS.map((k) => (
                <div key={`R-${k}`} className="flex items-center gap-2">
                  ...
                  <AutocompleteInput
                    id={`rolland-red-${k}`}
                    value={redRoster[k]}
                    onChange={(val) => setRedRoster((prev) => ({ ...prev, [k]: val }))}
                    sourceList={allStreamers}
                    type="streamer"
                    placeholder={`Red ${LINE_LABELS[k]}`}
                    className="flex-1 min-w-0"
                    inputClassName="h-[34px] bg-[#12121a] border border-[rgba(239,68,68,0.25)] rounded-full text-[12px] focus:border-[#ef4444]/60"
                  />
                </div>
              ))}
            </div>
              ...
              {LINE_KEYS.map((k) => (
                <div key={`B-${k}`} className="flex items-center gap-2">
                  ...
                  <AutocompleteInput
                    id={`rolland-blue-${k}`}
                    value={blueRoster[k]}
                    onChange={(val) => setBlueRoster((prev) => ({ ...prev, [k]: val }))}
                    sourceList={allStreamers}
                    type="streamer"
                    placeholder={`Blue ${LINE_LABELS[k]}`}
                    className="flex-1 min-w-0"
                    inputClassName="h-[34px] bg-[#12121a] border border-[rgba(59,130,246,0.25)] rounded-full text-[12px] focus:border-[#3b82f6]/60"
                  />
                </div>
              ))}
            </div>
