...
export function getInitialChampionCatalog(): DDragonChampionItem[] {
  ...
  for (const [name, positions] of Object.entries(KNOWN_CHAMPION_POSITIONS)) {
    if (!map.has(name)) {
      ...
      const isApc = positions.includes('ADC') && ['빅토르', '신드라', '제라스', '직스', '스웨인', '카서스', '세라핀', '베이가', '벨코즈', '브랜드', '하이머딩거', '흐웨이', '멜', '카시오페아'].includes(name);
      map.set(name, {
        id: en,
        name,
        enName: en,
        title: '리그 오브 레전드 챔피언',
        tags: positions.includes('ADC') ? ['Marksman', 'Mage'] : ['Fighter'],
        positions,
        isApc,
      });
    }
